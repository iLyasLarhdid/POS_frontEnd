...
const FirebaseRegister = ({ ...others }) => {
    ...
    const registerSubmit = (values) => {
        console.log(values);
        const url = `${config.host}/api/v1/users/register`;
        ...
        fetch(url , {
            ...
            body: JSON.stringify({firstName,lastName,userName,email,password,cityId,role})
        }).then(response=>{
            if(!response.ok){ throw Error("something went wrong")}
            return response.json()})
            .then(data=>{
                message.success("check your email for confirmation");
                new Promise((resolve)=>setTimeout(resolve,1500)).then(()=>{history.push("/login")});
            }).catch((err)=>{
