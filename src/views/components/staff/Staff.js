...
const fetchData = async (key)=>{
    ...
    const res = await fetch(`${host}/api/v1/staff/page/${page}`,{
        headers: {
...
//todo : you should put this in a component of its own so that you dont have to call it everytime
...
//the one only one that can access this page is the admin or the super admin and can see all their staff based on thier _id that is stored in the cookie (think of a better way not to store it in the cookies)

//we will show the users and their monthly paiments (i think the courier gets payed by the package and the secrutary by the month) and the number of days they have worked, as well as who's online right now

const Staff = () =>{
    const [satff,setStaff] =  useState(forumData);
    ...
    if(!cookies.ilyToken ||cookies.ilyToken ==null )
        ...
        const {data,isLoading,error} = useQuery(['forums',pageNumber,token],fetchData,{keepPreviousData:true});
